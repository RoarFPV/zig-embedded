...
pub fn build(b: *std.build.Builder) void {
    //const mode = b.standardTargetOptions();
    //const mode = b.standardReleaseOptions();

    const exe = b.addExecutable(
        "STM32F7x2-firmware.elf",
        comptime root() ++ "src/main.zig",
    );

    exe.addIncludePath(comptime root() ++ "src");
    exe.setLinkerScriptPath(.{ .path = comptime root() ++ "src/STM32F7x2/linker.ld" });
...
fn buildRegz(b: *std.build.Builder, comptime svdFile: []const u8, comptime registersPath: []const u8) *std.build.Step {
    ...
    const cmd = [_][]const u8{ comptime root() ++ "libs/regz/zig-out/bin/regz", comptime root() ++ svdFile, "-o", comptime root() ++ registersPath ++ "/registers.zig" };
